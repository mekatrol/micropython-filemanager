...
const runDeviceScript = async (board: PyDeviceConnection, body: string): Promise<string> => {
...
export const listDeviceEntries = async (board: PyDeviceConnection): Promise<FileEntry[]> => {
  const script = `
import os
try:
...
export const readDeviceFile = async (board: PyDeviceConnection, relativePath: string): Promise<Buffer> => {
...
export const writeDeviceFile = async (board: PyDeviceConnection, relativePath: string, content: Buffer): Promise<void> => {
  const absolutePath = toDeviceAbsolutePath(relativePath);
...
export const createDeviceDirectory = async (board: PyDeviceConnection, relativePath: string): Promise<void> => {
  const absolutePath = toDeviceAbsolutePath(relativePath);
  const script = `
import os
...
export const renameDevicePath = async (board: PyDeviceConnection, fromRelativePath: string, toRelativePath: string): Promise<void> => {
...
export const deleteDevicePath = async (board: PyDeviceConnection, relativePath: string): Promise<void> => {
  const absolutePath = toDeviceAbsolutePath(relativePath);
  const script = `
import os
