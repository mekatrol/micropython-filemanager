...
class ReplViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  private async handleWebviewMessage(message: WebviewMessage): Promise<void> {
    ...
    if (message.type === 'clear') {
      ...
      this.renderConnectedIntroForDevice(message.deviceId);
      this.postState();
      return;
    }
...
<head>
  ...
  <style>
    ...
    .clear { border: none; background: transparent; color: inherit; cursor: pointer; font-size: 13px; line-height: 1; padding: 0 2px; opacity: 0.8; }
  ...
  <script nonce="${nonce}">
    ...
    const render = () => {
      ...
      for (const device of currentState.devices) {
        ...
        clearButton.title = 'Clear ' + device.deviceId;
        clearButton.addEventListener('click', (event) => {
          event.stopPropagation();
          vscode.postMessage({ type: 'clear', deviceId: device.deviceId });
        });
