...
class DeviceSyncModel {
  ...
  async openSyncFiles(node?: SyncNode): Promise<void> {
    ...
    const renderSyncViewHtml = (): string =>
      this.renderDeviceFileSyncHtml(panel.webview, deviceId, currentRows, currentHasDifferences);
  ...
  private async pickSyncOperations(
    ...
    panel.webview.html = this.renderSyncPreviewHtml(panel.webview, titleText, directionText, rows);
  ...
  private renderDeviceFileSyncHtml(
    webview: vscode.Webview,
    deviceId: string,
